...
| §5.1 parans | Approved, geometry specified (`03-astro-calculation-spec.md` §19) | **Implemented** (`src/astro/parans.ts`, `src/astro/paran-proximity.ts`, wired into `select-influences.ts`/`internal-score.ts`/`route.ts`). Baseline-instant only, not tracked across birth-time uncertainty scenarios (documented scope decision, `docs/DECISIONS.md`). This closes the v0.2 framework. **Unrelated finding from the same investigation, also fixed the same day:** the pre-existing city-to-line distance computation was ~7.4s per scenario instant for the full city dataset (~22s+ per request) and had a latent correctness bug; both fixed, ~5.9x overall speedup measured locally -- see `docs/DECISIONS.md`'s "FIXED: pre-existing city-to-line distance computation..." entry for the full detail. Production (Vercel) timing not yet independently verified. |
