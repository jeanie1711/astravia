...
**Unrelated but urgent, found while perf-checking this feature:** the existing (pre-dating this framework) city-to-line distance computation (`src/astro/city-proximity.ts`'s `distanceToPolylineKm`, used for every ASC/DSC line) measures at roughly 7.4 seconds per scenario instant for the full ~955-city dataset -- brute-force checking every city against every one of the ~250 sampled points on every ASC/DSC polyline, no spatial pruning. A full `/api/calculate` request computes three such instants (lower/baseline/upper, per G007), so this alone is on the order of 20+ seconds before any scoring happens -- likely at or beyond typical Vercel serverless function timeouts. Parans add under 100ms and are not the cause. This was never verified before (Milestone 4's "performance verification" item was flagged pending from the start of the project) and should be treated as a separate, likely higher-priority item than anything else in this proposal.
