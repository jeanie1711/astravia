...
4. ~~The paran calculation module and its own Golden Tests, then wiring into scoring.~~ **Done 2026-09-05** — `src/astro/parans.ts` (geometry, tested against hand-computed cases), `src/astro/paran-proximity.ts` (city distance), wired into `select-influences.ts`/`internal-score.ts`/`stability.ts`/`score-city.ts`/the interpretation layer/`route.ts`. This closes the v0.2 framework in full.

**Unrelated but urgent, found while perf-checking this feature -- fixed the same day.** The existing (pre-dating this framework) city-to-line distance computation was ~7.4s per scenario instant for the full ~955-city dataset (~22s+ per request before any scoring). Two distinct bugs were found and fixed: `distanceToSegmentKm` had a latent correctness bug (its along-track formula used `acos`, which loses sign information and could wrongly accept a point "behind" a segment's endpoint as if it were on the segment), and `distanceToPolylineKm`/`computeCityInfluencesAcrossScenarios` had two separate O(n) and O(n²) performance issues, both fixed with exact (non-approximating) techniques verified by a brute-force cross-check test. Net effect: ~5.9x speedup measured locally (~36s → ~6.1s for the worst-case OVERALL-goal request). Full detail, including why the correctness bug was never caught before, in `docs/DECISIONS.md`. Production (Vercel) timing has not been independently verified.
